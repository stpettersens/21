...
class Dealer 
	...
	_hit: (cards) ->
		...
		return new Card(Card.getImage(card), @pos, 10, @game)
	...
	shuffle: (cards) ->
		if cards.getPlayed() == 0 or cards.getPlayed() >= 45
			SoundEffects.play(@game, "shuffle")
			Debug.emit(@debug, "-------------------------------------------------------")
	...
	receiveCards: () ->
		cardA = new Card(Card.getImage("c"), @pos, 10, @game)
		@pos += 90
		cardB = new Card(Card.getImage(@cards[1]), @pos, 10, @game)
		@index += 2
		return [cardA, cardB]
	...
	revealFirstCard: () ->
		SoundEffects.play(@game, "reveal")
		return new Card(Card.getImage(@cards[0]), 225, 10, @game)
